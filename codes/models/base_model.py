...
class BaseModel():
    ...
    def load_network_bsn2nbsn(self, load_path, network, strict=True):
        ...
        load_net_clean = OrderedDict()  # remove unnecessary 'module.'
        for k, v in load_net.items():
            if k.startswith('module.'): #不会进这个分支的。
                if k[7:][:5]=="nin_A" or k[7:][:5]=="nin_B" or k[7:][:5]=="nin_C":
                    continue
                if k[7:][5:].startswith('dec1'):
                    if k[7:][5+5:].startswith('conv4'):
                        continue
                load_net_clean[k[7+5:]] = v
            else:
                if k[:5]=="nin_A" or k[:5]=="nin_B" or k[:5]=="nin_C":
                    continue
                if k[5:].startswith('dec1'):
                    if k[5+5:].startswith('conv4'):
                        continue
                load_net_clean[k[5:]] = v

        ori = network.state_dict()
        ori.update(load_net_clean)
        network.load_state_dict(ori, strict=strict)

    def save_training_state(self, epoch, iter_step, network_label):
